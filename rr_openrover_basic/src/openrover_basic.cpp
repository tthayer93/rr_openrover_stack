...
const float ODOM_SMOOTHING = 100.0;
...
OpenRover::OpenRover( ros::NodeHandle &_nh, ros::NodeHandle &_nh_priv ) :
    port_("/dev/ttyUSB0"),
    baud_(57600),
    fast_rate_(30.0), //10Hz Total Serial data is limited to 66 msgs/second
...
bool OpenRover::start()
{
	...
	cmd_vel_sub_remote = nh.subscribe("/cmd_vel/laptop/managed", 1, &OpenRover::cmdVelCB_remote, this);
    
    battery_status_publisher = nh.advertise<std_msgs::Int32>("/battery/status",1);

    left_motor_temp_publisher = nh.advertise<std_msgs::Int32>("/temp/motor/left",1);

    right_motor_temp_publisher = nh.advertise<std_msgs::Int32>("/temp/motor/right",1);
...
void OpenRover::cmdVelCB_remote(const geometry_msgs::TwistStamped::ConstPtr& msg)
{//converts from cmd_vel (m/s and radians/s) into motor speed commands
    float left_motor_speed, right_motor_speed;
    int flipper_motor_speed;
    int motor_speed_deadband_scaled;
...
void OpenRover::publishSlowRateData()
{
    ...
    battery_status.data = slow_msg.reg_robot_rel_soc_a;   
    battery_status_publisher.publish(battery_status);

    left_motor_temp.data = slow_msg.reg_motor_temp_left;
    left_motor_temp_publisher.publish(left_motor_temp); 

    right_motor_temp.data = slow_msg.reg_motor_temp_right;
    right_motor_temp_publisher.publish(right_motor_temp);
